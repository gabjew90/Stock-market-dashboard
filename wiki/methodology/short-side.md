...
Dr. Wish is a long-side trader who spends bear markets mostly in cash; the short side is a minority activity he has nonetheless documented in detail — a scan for stocks in confirmed down-trends (the **submarine scan**), a topping-pattern scan, the small SQQQ hedge on a new QQQ down-trend, and two options overlays (protective puts, covered calls). This page collects all of it. The default that governs when he reaches for any of it — cash over short, by age and temperament — stays on [risk-and-cash](risk-and-cash.md#cash-over-short--the-age-based-default).
...
- [Exits](../playbooks/exits.md#step-2--choose-stop-loss-order-or-protective-put) — the stop-vs-put decision in procedure form
- [Market-state playbook](../playbooks/market-state.md#step-2--read-the-qqq-short-term-count) — when the small SQQQ hedge is on the table
- [Moving-average rules](moving-average-rules.md) — Stage 4 and the weekly BWR pattern the submarine scan looks for
- [Glossary](glossary.md) — submarine scan, BWR, DITM
